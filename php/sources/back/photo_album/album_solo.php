...
{ 
    ...
    if ($photo=='') {
        ...
        echo "<li class='active'><a href='".$photo."'><i class='icon-pictures'></i> photos</a></li>";
    }
...
$id_album=$_GET['id'];
?>
                    <a href="index.php" class="btn btn-primary">Retour aux albums</a>
                    <a href="4_ajout_photo2.php?id=<?php echo $id_album;?>" class="btn btn-primary">Ajouter photo</a>
                    <form action="supp_photo.php" method="post" enctype="multipart/form-data"></p>
    ...
    for (var i = 0; i < elements.length; i++) {
        if (elements[i].type == 'checkbox' && elements[i].name == name) {
            elements[i].checked = ref.checked;
                    	...
                    	<span><i class="icon-pictures"></i> Galerie photos </span>
                    </div>
                    <div class="mws-panel-body">Séléctionner toutes les photos <input onclick="CocheTout(this, 'option[]');" type="checkbox">
                		<ul class="thumbnails mws-gallery">

<?php

$sql = "SELECT * FROM  photo WHERE album =$id_album ORDER BY id DESC"; 
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error()); 
while($mot = mysql_fetch_assoc($req)) 
{ 
$image=$mot['lien'];
...
?>
                			<li>
                            	<span class="thumbnail"><img src="../../images/<?php echo $image;?>" alt="" height="113px"></span>
                                <input type="checkbox" name="option[]" value="<?php echo $id;?>">
                                <span class="mws-gallery-overlay">
                                    <a href="../../images/<?php echo $image;?>" rel="prettyPhoto[gallery1]" class="mws-gallery-btn"><i class="icon-search"></i></a>
                                </span>
                			</li>
<?php
}
?>  			
            ...
            <div class="mws-panel grid_2">
                <input type="submit" class="btn btn-primary" value="supprimer photos" name="suppr" onclick="return confirm('êtes vous sur de vouloir supprimer ces photos ?');">
                <input type="submit" class="btn btn-primary" value="Choisir couverture de l'album" name="thumb">
                </form>
            </div>
                <!-- Panels End -->
            </div>
