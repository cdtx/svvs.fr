...
if ($changement_fly==4) {
     $sql = "UPDATE  `agenda` SET  `titre` =  '$titre',`date` =  '$datetime',`texte` =  '$texte' WHERE  `id` =$id;";
...
$lien='/images/articles/'.$fichier;
     ...
     $sql = "UPDATE  `agenda` SET  `titre` =  '$titre',`photo` =  '$lien',`date` =  '$datetime',`texte` =  '$texte' WHERE  `id` =$id;";
     $req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
     echo "<script type='text/javascript'>document.location.replace('index.php?=modif');</script>";
